...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function render(result){
    const prevPercent = parseFloat(percentText?.textContent) || 0;
    animateNumber(percentText, prevPercent, result.percent);
    ...
    if (finishDays){
      finishDays.textContent = (result.remaining === 0)
        ? "Completed âœ…"
        : `${result.daysRoundedUp} day(s)`;
    }

    if (finishDate){
      finishDate.textContent = (result.remaining === 0)
        ? formatDate(Date.now())
        : formatDate(result.finishTs);
    }

    if (progressLabel){
      progressLabel.textContent = (result.remaining === 0)
        ? "You finished this book!"
        : `Keep going â€” ${result.remaining} page(s) left.`;
    }
    ...
    setSuggestions(
      result.remaining === 0
        ...
        : [
            `If you read ${result.speed} pages/day, youâ€™ll finish in about ${result.daysRoundedUp} day(s).`,
      ...
      li.innerHTML = `
        <div>
          <div class="tracker_saved_title">
            ${item.bookName} â€” ${item.percent}% completed
          </div>
          <div class="tracker_saved_meta">
            ${item.read}/${item.total} pages â€¢ ${item.speed} pages/day â€¢
            Finish: ${item.remaining === 0 ? "Done" : formatDate(item.finishTs)} â€¢
  ...
  clearSavedBtn?.addEventListener("click", () => {
    clearSavedList();
    setError("Saved list cleared âœ…");
    setTimeout(() => setError(""), 1200);
  });
